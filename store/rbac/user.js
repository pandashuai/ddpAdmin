...
export const mutations = {
    ...
    GET_ITEM_LIST_FAILURE(state, action) {
        state.root = true;
        state.current = {
            user: '',
            password: '123456',
...
export const actions = {
    ...
    async create({ commit }, params) {
        try {
            return await Service.post('/rbac/user/create', params);
        } catch (err) {
            return { err }
        }
    },
    ...
    async lists({ commit }, params = { per_page: 1000 }) {
        ...
        try {
            let res = await Service.get('/rbac/user/lists', { params })
    ...
    async del({ commit }, params) {
        try {
            return await Service.post('/rbac/user/del', params);
        } catch (err) {
            return { err }
        }
    },
    ...
    async find({ commit }, params) {
        commit('EDIT_ROOT_STATUS', false);
        try {
            let res = await Service.get('/rbac/user/find', { params })
            if (res.data && res.data.code === 1) {
    ...
    async edit({ commit }, params) {
        try {
            return await Service.post('/rbac/user/edit', params);
        } catch (err) {
            return { err }
        }
    }
