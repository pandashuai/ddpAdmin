...
export const actions = {
    ...
    async create({ commit }, params) {
        try {
            return await Service.post('/rbac/role/create', params);
        } catch (err) {
            return { err }
        }
    },
    ...
    async lists({ commit }, params = { per_page: 1000 }) {
        ...
        try {
            let res = await Service.get('/rbac/role/lists', { params })
    ...
    async del({ commit }, params) {
        try {
            return await Service.post('/rbac/role/del', params);
        } catch (err) {
            return { err }
        }
    },
    ...
    async find({ commit }, params) {
        commit('EDIT_ROOT_STATUS', false);
        try {
            let res = await Service.get('/rbac/role/find', { params })
            if (res.data && res.data.code === 1) {
    ...
    async edit({ commit }, params) {
        try {
            return await Service.post('/rbac/role/edit', params);
        } catch (err) {
            return { err }
        }
    }
